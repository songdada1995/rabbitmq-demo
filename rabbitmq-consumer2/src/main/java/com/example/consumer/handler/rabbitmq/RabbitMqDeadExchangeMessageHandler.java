...
public class RabbitMqDeadExchangeMessageHandler {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = RabbitMqConstant.DEAD_LETTER_QUEUE_2),
            exchange = @Exchange(
                    value = RabbitMqConstant.DEAD_LETTER_EXCHANGE,
                    ignoreDeclarationExceptions = "true"
            ),
            key = {RabbitMqConstant.DEAD_LETTER_QUEUE_2_ROUTING_KEY}),
            containerFactory = "manualAckRabbitListenerContainerFactory")
    public void processDeadQueue2Message(Message message, Channel channel) throws Exception {
        log.info("======" + Thread.currentThread().getName() + "开始执行死信队列DeadQueue2======");
        ...
        try {
            ...
            log.info("======" + Thread.currentThread().getName() + "结束执行死信队列DeadQueue2======");
            log.info(RabbitMqConstant.DEAD_LETTER_QUEUE_2 + "====确认消息：" + message);
